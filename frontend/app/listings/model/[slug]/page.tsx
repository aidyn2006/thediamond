...
import { brandLabels, brandSlugs, formatTenge, storageLabel } from "@/lib/phones";
import { findModelHub, modelRuAlias, type ModelHub } from "@/lib/models";
...
export async function generateMetadata({
  ...
  const hub = findModelHub(await getPublicListings(), slug);
  if (!hub) return pageMetadata({ title: "Модель не найдена", index: false });
  return pageMetadata({
    title: title(hub),
    description: description(hub),
    path: `/listings/model/${slug}`,
  });
}
...
export default async function ModelHubPage({
  ...
  const { slug } = await params;
  // The hub only exists while somebody sells the model — a 404 afterwards is honest,
  // and the sitemap stops listing it at the next revalidate.
  const hub = findModelHub(await getPublicListings(), slug);
  if (!hub) notFound();
    ...
    <>
      ...
      <main id="main-content" className="mx-auto max-w-[1200px] px-6 py-8 md:px-10">
        <nav aria-label="Хлебные крошки" className="mb-4 text-13 text-text-dim">
          ...
          <Link href={`/listings/brand/${brandSlugs[hub.brand]}`} className="hover:text-text">
      ...
      <JsonLd
        data={[
          ...
          breadcrumbJsonLd([
            ...
            { name: brandLabels[hub.brand], path: `/listings/brand/${brandSlugs[hub.brand]}` },
            { name: hub.model, path: `/listings/model/${slug}` },
          ]),
          faqPageJsonLd(qa),
        ]}
      />
