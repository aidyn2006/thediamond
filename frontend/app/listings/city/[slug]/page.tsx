...
import { cityBySlug, cityPath, type CityInfo } from "@/lib/geo";
import { brandLabels, brandSlugs, formatTenge } from "@/lib/phones";
...
export async function generateMetadata({
  ...
  if (!city) return pageMetadata({ title: "Город не найден", index: false });
        ...
        {listings.length > 0 && (
          <div className="mt-4 flex flex-wrap gap-2 text-13">
            ...
            {LINKED_BRANDS.map((b) => (
              <Link
                key={b}
                href={`/listings/brand/${brandSlugs[b]}/${city.slug}`}
